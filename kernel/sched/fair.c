...
#ifdef CONFIG_SCHED_WALT
/* 1ms default for 12ms window size scaled to 1024 */
unsigned int sysctl_sched_min_task_util_for_boost = 85;
/* 0.68ms default for 12ms window size scaled to 1024 */
unsigned int sysctl_sched_min_task_util_for_colocation = 58;
			...
			if (is_gmod_enable() && prefer_top(p))
				...
				if (!(im_rendering(p) && prefer_sched_group(p)) &&
					...
					cpumask_clear_cpu(
						...
						rd->max_cap_orig_cpu : rd->mid_cap_orig_cpu,
						&p->cpus_suggested);
					cpumask_clear_cpu(rd->max_cap_orig_cpu, &p->cpus_suggested);
				}
			}
		} else {
